...
public:
    ...
    static std::unique_ptr<algos::order::Order> CreateOrderInstance(
            tests::TableConfig const& info) {
        return algos::CreateAndLoadAlgorithm<algos::order::Order>(GetParamMap(info));
    }
};

TEST_F(OrderTest, SmallDataset) {
    auto a = CreateOrderInstance(kOD_norm6);
    a->Execute();
