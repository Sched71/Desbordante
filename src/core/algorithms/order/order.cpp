...
void Order::CreateSingletonSortedPartitions() {
    ...
    for (unsigned int i = 0; i < data.size(); ++i) {
        ...
        sorted_partitions_.emplace(AttributeList{i}, SortedPartition(std::move(equivalence_classes),
    ...
    for (auto& [attr, partition] : sorted_partitions_) {
        if (partition.Size() == 1) {
            ...
            single_attributes_.erase(std::find(single_attributes_.begin(), single_attributes_.end(), attr));
        }
    }
    for (auto& [attr, partition] : sorted_partitions_) {
        if (partition.Size() == 1) {
